...
/**
 * @Author yajiewen
 * @Date 2022-04-05 08-14-36
 * @Description 修复交叠空间的更新(6个面用同种方法更新)而不是把z正方向单独更新
 */
public class ResourceHelper {

    // 资源(状态)更新(传入的spaceArrayList 是顶部空间去除后的, space 是被去除的放置空间, block是放置快,)
    public static void renewResource(ArrayList<Space> spaceArrayList, Space space, Block block, int[] availNum) {
        // 更新箱子数目
        ...
        // 求剩余空间(C2支持完全支撑)
        ArrayList<Space> newSpaceList = getLeftSpaceForCoveredSpace(space, block);
        ...
        for (int i = spaceArrayList.size() - 1; i >= 0; i--) {
            ...
            if (coverBlock != null) { // 有交叠则在list中删除该空间
                ...
                coveredSpaceList.addAll(getLeftSpaceForCoveredSpace(coverSpace, coverBlock)); // 更新该空间的剩余空间,并返回列表
            }
        }
    ...
    // 给一个空间和一个 block 求剩余空间(完全支撑)
    public static ArrayList<Space> getLeftSpaceFullySupport(Space space, Block block) {
        ...
        Space spaceX1 = new Space(x1SpaceCoordinate);
        if (!spaceX1.isZeroV()) {
        ...
        Space spaceX2 = new Space(x2SpaceCoordinate);
        if (!spaceX2.isZeroV()) {
        ...
        Space spaceY3 = new Space(y3SpaceCoordinate);
        if (!spaceY3.isZeroV()) {
        ...
        Space spaceY4 = new Space(y4SpaceCoordinate);
        if (!spaceY4.isZeroV()) {
        ...
        Space spaceZ5 = new Space(z5SpaceCoordinate);
        if (!spaceZ5.isZeroV()) {
     ...
     */
    public static ArrayList<Space> getLeftSpaceForCoveredSpace(Space space, Block block) {
        ...
        Space spaceZ6 = new Space(z6SpaceCoordinate);
        if (!spaceZ6.isZeroV()) {
