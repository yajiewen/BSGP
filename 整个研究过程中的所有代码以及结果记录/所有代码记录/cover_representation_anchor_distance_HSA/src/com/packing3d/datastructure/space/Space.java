...
/**
 * @Author yajiewen
 * @Date 2022-04-04 13-19-22
 * @Description 有anchor corner 和anchor distance 的space 数据结构
*/
public class Space{
    ...
    // 使用0点坐标 生成带 anchordistanse 和 corner 的空间
    public Space(double locationX, double locationY, double locationZ, double spaceLength, double spaceWidth, double spaceHigh, Problem problem) {
        ...
        // 定义八点距离
        double[][] eightCornerDistance = new double[8][3];
        // 计算八点距离(这里只求0123 这四个点,因为完全支撑不考虑悬浮放置)
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 3; j++){
                eightCornerDistance[i][j] = Math.abs(problem.containerEightCoordinate[i][j] - this.eightCoordinate[i][j]);
            }
            // corner distance 向量内排序
            Arrays.sort(eightCornerDistance[i]);
//            System.out.println(Arrays.toString(eightCornerDistance[i]));
        }
        ...
        for(int i = 1; i < 4; i++){
            for(int j = 0; j < 3; j++){
                if(eightCornerDistance[anchorCorner][j] > eightCornerDistance[i][j]){
                    anchorCorner = i;
                    break;
                }else if(eightCornerDistance[anchorCorner][j] < eightCornerDistance[i][j]){
                    break;
                }
            }
        }
        anchorDistance = new double[3];
        System.arraycopy(eightCornerDistance[anchorCorner], 0, anchorDistance, 0, 3);
//        System.out.println("anchor corner :" + anchorCorner);
//        System.out.println("anchor distance" + Arrays.toString(anchorDistance));
    }

    // 通过八点坐标生成空间 生成带 anchordistanse 和 corner 的空间2
...
//        System.out.println("anchor distance" + Arrays.toString(anchorDistance));
    ...
    public boolean isZeroV(){
        return spaceHigh == 0 || spaceWidth == 0 || spaceLength == 0;
    }
