...
//            singleThread();
    ...
    /**
     * @Author yajiewen
     * @Date 2023-02-22 21-45-57
     * @Description 服务器使用的单线程
    */
    ...
    public static void singleThread(String inputPath, String outputPath, String fileNum) throws IOException{
        ...
        // 定义一个变量保存 旋转后的块表的线程效果比正常线程好的次数
        int betterTims = 0;

        for (Problem problem : problemList) {

            ArrayList<State> stateList = biBSG(problem,InitHelper.RUN_TIME_SECOND_LIMIT * 1000, MinFillRate, isRuoYiGou);
            double fillRate = stateList.get(0).scheme.totalVolum / problem.containnerVolume;
            double fillRateRotated = stateList.get(1).scheme.totalVolum / problem.containnerVolume;

            if(fillRate > fillRateRotated){
                avaFillRate += fillRate;
                System.out.println("BR"+ p +" problem " + i +" : "+"正常填充率" + fillRate +" 旋转后填充率" + fillRateRotated);
            }else{
                avaFillRate += fillRateRotated;
                betterTims++;
                System.out.println("BR"+ p +" problem " + i +" : "+"正常填充率" + fillRate +" 旋转后填充率" + fillRateRotated + " 旋转后更好");
            }

            if(InitHelper.STATE_CHECK){
                StateHelper.stateCheck(stateList.get(0),problem);
                StateHelper.stateCheck(stateList.get(1),problem);
            }
            i++;
            // 每个问题结束后gc前
            System.gc();
    ...
    */
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (Problem problem : problemList) {
                        ...
                        ArrayList<State> stateList = biBSG(problem,timeLimit * 1000, MinFillRate, isRuoYiGou);
                        double fillRate = stateList.get(0).scheme.totalVolum / problem.containnerVolume;
                        double fillRateRotated = stateList.get(1).scheme.totalVolum / problem.containnerVolume;
    ...
    /**
     * @Author yajiewen
     * @Date 2023-02-22 21-16-15
     * @Description 单程的BiBSGI
    */
    ...
    public static void singleThread() throws IOException{
        ...
        double avaFillRate = 0;
        int betterTims = 0,i = 0;
        for(Problem problem : problems){
            ArrayList<State> stateList = biBSG(problem,InitHelper.RUN_TIME_SECOND_LIMIT * 1000, MinFillRate, false);
            double fillRate = stateList.get(0).scheme.totalVolum / problem.containnerVolume;
            double fillRateRotated = stateList.get(1).scheme.totalVolum / problem.containnerVolume;

            if(fillRate > fillRateRotated){
                avaFillRate += fillRate;
                System.out.println(" problem " + i +" : "+"正常填充率" + fillRate +" 旋转后填充率" + fillRateRotated);
            }else{
                avaFillRate += fillRateRotated;
                betterTims++;
                System.out.println(" problem " + i +" : "+"正常填充率" + fillRate +" 旋转后填充率" + fillRateRotated + " 旋转后更好");
            }
        ...
        System.out.println("平均利用率为：" + avaFillRate);
    }
...
//            rate += state.scheme.totalVolum / problem.containnerVolume;
//            if(InitHelper.STATE_CHECK){
...
//        ArrayList<Block> blockList = BlockGenerator.guillotineBlocks(problem,MaxBlocks,MinFillRate);
        ...
        if(isRuoYiGou){
            blockList = BlockGenerator.simpleBlocks(problem,MaxBlocks);
    ...
    /**
     * @Author yajiewen
     * @Date 2023-02-22 20-50-48
     * @Description 并行BSGI
    */
    public static ArrayList<State> biBSG(Problem problem,int timeLimit,double MinFillRate, boolean isRuoYiGou){
        // 开辟一个final的对象数组,用于保存匿名内部类的里面的结果
        State[] bestState = new State[1];
        State[] bestStateRotated = new State[1];
        // 分别记录两个线程的最好解
        ArrayList<State> returnStates = new ArrayList<>();
        int MaxBlocks = 10000;

        ArrayList<Block> blockList,rotatedBlockList;
        if(isRuoYiGou){
            blockList = BlockGenerator.simpleBlocks_Zhang(problem);
            System.out.println(blockList.size() + "简单快生成完毕");
            rotatedBlockList = BlockGenerator.getRotatedBlockList(blockList,problem);
            System.out.println("旋转后的块数：" + rotatedBlockList.size());
        }else{
            // 生成正常方向的块
            blockList = BlockGenerator.generalPackingBlocks(problem,MaxBlocks,MinFillRate,0.98);
            // 生成旋转的块
            rotatedBlockList = BlockGenerator.getRotatedBlockList(blockList,problem);
            System.out.println("旋转后的块数：" + rotatedBlockList.size());
        }
        ...
        // 旋转块线程
        Thread threadRotated = new Thread(new Runnable() {
            @Override
            public void run() {
                int w = 1;
                while(true){
                    beamSearch(problem,rotatedBlockList,w,bestStateRotated);
                    w =(int) Math.ceil(Math.sqrt(2) * w);
                }
            }
        });
        ...
        returnStates.add(bestState[0]);
        returnStates.add(bestStateRotated[0]);
