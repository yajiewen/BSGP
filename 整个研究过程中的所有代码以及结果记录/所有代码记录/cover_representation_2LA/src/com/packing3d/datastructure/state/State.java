...
public class State {
    ...
    public State(Problem problem,ArrayList<Block> blockList){
        ...
        // 初始化剩余空间栈
        spaceArrayList.add(new Space(0,0,0, problem.containnerLength, problem.containnerWidth, problem.containnerHigh));
        // 初始化块表,不能用引用,因为后面要删除
    ...
    public static State getNewState(State stateOld,Block block){
        ...
        // 栈顶空间出栈
        Space space = newState.spaceArrayList.remove(newState.spaceArrayList.size() - 1);
        // 更新newState
        newState.scheme.addPut(new Put(block,space));
        // 更新资源相关
        ResourceHelper.renewResource(newState.spaceArrayList, space, block, newState.availBox);
        // 块列表更新
        newState.blockList.remove(block); // 这个不能留会有块丢失
        // 删除剩余箱子不可生成的块
    ...
    public State cloneObj(){
        ...
        // 复制空间栈内容
//        for (Space space : this.spaceArrayList) {
//            state.spaceArrayList.add(space);
//        }
