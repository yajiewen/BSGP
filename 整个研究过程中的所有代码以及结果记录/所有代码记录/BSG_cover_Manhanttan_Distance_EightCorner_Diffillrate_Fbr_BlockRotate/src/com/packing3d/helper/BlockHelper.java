...
//        }
     ...
     * 8-23 更新,增加了单个箱子生成的块，解决块丢失，增加删除相似块优化
     */
    ...
    public static ArrayList<Block> generateGuillotineBlocksNotFullySupported(Problem problem,int MaxBlocks,double MinFillRate){
        ...
        // 删除相似块
        blockList = deleteSimilarBlock(blockList,5);
    ...
    /**
     * @Author yajiewen
     * @Date 2022-10-02 09-37-41
     * @Description 删除相似的块
    */

    public static ArrayList<Block> deleteSimilarBlock(ArrayList<Block> blockList,double similarScore){
        // 为了删除相似块且保留体积大的块需要先排序
        blockList.sort(new Comparator<Block>() {
            @Override
            public int compare(Block o1, Block o2) {
                if (o2.blockVolum > o1.blockVolum) {
        ...
        for (Block block : blockList) {
            if(returnBlockList.isEmpty()){
                returnBlockList.add(block);
            }else{
                boolean isAdd = true;
                for (Block block1 : returnBlockList) {
                    if(getSimilarScore(block,block1) < similarScore){
                        isAdd = false;
                        break;
                    }
                }
    ...
    public static double getSimilarScore(Block blockA,Block blockB){
        return Math.abs(blockA.blockLenght - blockB.blockLenght) + Math.abs(blockA.blockWidth - blockB.blockWidth) + Math.abs(blockA.blockHigh - blockB.blockHigh);
    }
    ...
    */
    ...
    public static int scoreFunction(Space space, Block block, State state){
        if((block.blockLenght <= space.spaceLength &&
                block.blockWidth <= space.spaceWidth &&
                block.blockHigh <= space.spaceHigh) || (block.blockHigh <= space.spaceHigh &&
                block.blockLenght <= space.spaceWidth && block.blockWidth <= space.spaceLength)){
            // 需要旋转的话直接旋转
            if(block.isNeedRoate(space)){
                block.blockRotate();
            }
            // 判断剩余箱子数目是否能满足这个块
            boolean isOk = true;
