...
public class Block {
    ...
    /**
     * @Author yajiewen
     * @Date 2022-09-30 20-05-50
     * @Description 添加块旋转
    */
    public void blockRotate(){
        double term = blockLenght;
        blockLenght = blockWidth;
        blockWidth = term;
    }

    /**
     * @Author yajiewen
     * @Date 2022-09-30 20-29-50
     * @Description 判断放入空间，块是否需要旋转
    */
    public boolean isNeedRoate(Space space){
        return (blockLenght <= space.spaceWidth && blockWidth <= space.spaceLength) &&
                (!(blockLenght <= space.spaceLength && blockWidth <= space.spaceWidth));
    }
    ...
    public Block cloneObj(){
        ...
        // 复制每种箱子所需数目
        System.arraycopy(this.neededBoxNumOfEachType, 0, block.neededBoxNumOfEachType, 0, this.neededBoxNumOfEachType.length);
        // 复制八点坐标
        for(int i = 0; i < 8; i++){
            System.arraycopy(this.eightCoordinate[i], 0, block.eightCoordinate[i], 0, 3);
        }
        // 复制x，y，z范围
        block.xRange[0] = this.xRange[0];
        block.xRange[1] = this.xRange[1];
        block.yRange[0] = this.yRange[0];
        block.yRange[1] = this.yRange[1];
        block.zRange[0] = this.zRange[0];
        block.zRange[1] = this.zRange[1];
        return block;
    }
