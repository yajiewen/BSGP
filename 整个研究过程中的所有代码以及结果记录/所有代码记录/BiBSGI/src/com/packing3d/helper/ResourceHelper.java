...
//        renewSpaceListByEightCorner(block,selectedSpaceIndex,state.spaceArrayList,state.problem);
        // 更新块表
     ...
     */
    public static void renewSpaceListByEightCorner(Block block, int selectedSpaceIndex, ArrayList<Space> spaceArrayList, Problem problem){
        // 从空间列表中删除被选中的空间
        Space space = spaceArrayList.remove(selectedSpaceIndex);
        // 把块放在空间的anchor点,根据空间anchor点坐标更新块的八点坐标
        block.initCoordinateAndRangeByAnchorCorner(space);
        // 根据八点坐标得到当前被放置空间的剩余空间
        ArrayList<Space> newSpaceList = renewSpaceFullySupport(space,block,problem);
        // 更新 其余交叠空间
        ...
        for (int i = spaceArrayList.size() - 1; i >= 0; i--) {
            ...
            if(coverBlock != null){ // 有交叠则在list中删除该空间
                ...
                // 更新该空间的剩余空间,并返回列表
                coveredSpaceList.addAll(renewSpaceFullySupport(coverSpace,coverBlock,problem));
            }
        }
    ...
    public static ArrayList<Space> renewSpaceFullySupport(Space space, Block block, Problem problem){
     ...
     * @Description 再一次重大修复,先判断 两个面是否重合,重合则说明该方向生成体积为0 非完全支撑
     */
