 ...
 * (K3) how to select a free space; 选择anchor distance 最小的空间
 * (K4) how to select a block; 选择f(b,r)分数最大的块
 * (K5) how to place the selected block into the selected space and update the list of free space; 块放在anchor corner ,使用八点坐标更新发求剩余空间,同时支持完全支撑
...
//        State state = algorithm2LA(problem,gPBlockList,isStrong);
        State state = algorithmG2LA(problem,gPBlockList);
    ...
    // G2LA算法
    public static State algorithmG2LA(Problem problem, ArrayList<Block> blockList){
        // 得到初始化的state
        int w = 1; // 定义搜索宽度
        int depth = 2;// 向前深度
        ...
        while(System.currentTimeMillis() - start <= 6000 * 100){
...
//                    System.out.println("当前bestComplete:" + bestCompleteState.scheme.totalVolum / (problem.containnerLength * problem.containnerWidth * problem.containnerHigh));
            ...
            bestCompleteStateList.add(state);
        }
        return findBestCompleteState(bestCompleteStateList);
    }
...
//        System.out.println("当前complete个数"+ completeStateList.size());
    ...
    public static void greedyForCompleteState(State completeState){
        while(!completeState.spaceArrayList.isEmpty()){
            ...
            // 查找可行快表(限制数目为1)
            ArrayList<Block> viableBlockList = BlockHelper.searchViableBlockWithNumLimitAndFbr(completeState.spaceArrayList.get(selectedSpaceIndex), completeState,1);
            if(!viableBlockList.isEmpty()){
                // 更新state
