...
//            State state = algorithmBSG(problem,30 * 1000,MinFillRate,false);
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (Problem problem : problemList) {
                        State state = algorithmBSG(problem,500 * 1000,MinFillRate, isRuoYiGou);
...
//        ArrayList<Block> blockList = BlockHelper.generateGeneralCuttingBlocksNotFullySupported(problem,MaxBlocks,MinFillRate);
        ArrayList<Block> blockList = BlockHelper.generateGuillotineBlocksNotFullySupported(problem,MaxBlocks,MinFillRate);
//        if(isRuoYiGou){
//            blockList = BlockHelper.generateGuillotineBlocksNotFullySupported(problem,MaxBlocks,MinFillRate);
//        }else{
//            blockList = BlockHelper.generateGeneralCuttingBlocksNotFullySupported(problem,MaxBlocks,MinFillRate);
//        }
    ...
    public static void beamSearch(Problem problem, ArrayList<Block> blockList, int w, State[] bestState){
        ...
        while(!stateList.isEmpty()){
            ...
            for(State state : stateList){
                if(false){ // 新论文里面没有判断这一步离谱，并且去掉后效果更好
            ...
            if(!successors.isEmpty()){
                ...
                // 删除相似状态(目前没看出来有什么区别)
//                successors = removeSimilarStatesByVolume(successors);
                successors = removeSimilarStates(successors);
//                System.out.println("successors = " +successors.size() + "w=" + w);
    ...
    */
    public static double greedy(State state,final State[] bestState,State oriState){
        ...
        while(!state.spaceArrayList.isEmpty()){
            ...
            ArrayList<Block> viableBlockList = BlockHelper.searchViableBlockWithNumLimitByFbr(state.spaceArrayList.get(selectedSpaceIndex), state,1);
...
//        System.out.println("greedy耗时：" + (System.currentTimeMillis() - start) + "ms");
...
//        System.out.println("删除相似state耗时：" +(System.currentTimeMillis() - start) +"ms");
...
//        ArrayList<Block> viableBlockList = BlockHelper.searchViableBlockWithNumLimit(state.spaceArrayList.get(selectedSpaceIndex),state.blockList,state.availBox,w);
        ArrayList<Block> viableBlockList = BlockHelper.searchViableBlockWithNumLimitByFbr(state.spaceArrayList.get(selectedSpaceIndex), state, w);
        for(Block block :viableBlockList){
            State newState = State.getNewState(state,block,selectedSpaceIndex);
            succ.add(newState);
        }
//        System.out.println("expend 耗时" + (System.currentTimeMillis() - start) + "ms");
        return succ;
    }
}
