...
/**
 * @Author yajiewen
 * @Date 2022-09-24 12-06-56
 * @Description 优化代码
 * 删除了合并剩余空间，剩余空间的合并根据有没有找到块来决定
 */
public class ResourceHelper {
    // 资源(状态)更新(传入的spaceArrayList 是顶部空间去除后的, space 是被去除的放置空间, block是放置快,)
    public static void renewResource(State state, Space space, Block block){
        // 更新箱子数目
        renewAvailBoxNum(block, state.availBox);
        // 放置块并更新剩余空间
        renewSpaceList(block,space,state.spaceArrayList,state.problem);
        // 剩余空间合并
//        mergeSpace(state.spaceArrayList, state.problem);
        // 更新块表
        renewBlockList(state);
    }
    /**
     * @Author yajiewen
     * @Date 2022-08-21 15-55-58
     * @Description 剩余空间合并
    */
    public static void mergeSpace(ArrayList<Space> spaceArrayList, Problem problem){
        ArrayList<Space> mergedSpaceList = new ArrayList<>();
        for(int i = spaceArrayList.size() - 1; i > 0; i--){
            Space spaceA = spaceArrayList.get(i);
            for(int j = i - 1; j >= 0; j--){
                Space spaceB  = spaceArrayList.get(j);
                // x 方向合并
                if((Arrays.equals(spaceA.eightCoordinate[1],spaceB.eightCoordinate[0]) &&
                Arrays.equals(spaceA.eightCoordinate[2],spaceB.eightCoordinate[3] )&&
                Arrays.equals(spaceA.eightCoordinate[5],spaceB.eightCoordinate[4] )&&
                Arrays.equals(spaceA.eightCoordinate[6],spaceB.eightCoordinate[7])) ||
                (Arrays.equals(spaceB.eightCoordinate[1],spaceA.eightCoordinate[0]) &&
                Arrays.equals(spaceB.eightCoordinate[2],spaceA.eightCoordinate[3] )&&
                Arrays.equals(spaceB.eightCoordinate[5],spaceA.eightCoordinate[4] )&&
                Arrays.equals(spaceB.eightCoordinate[6],spaceA.eightCoordinate[7]))){
                    Space mergedSpace = new Space(Math.min(spaceA.eightCoordinate[0][0],spaceB.eightCoordinate[0][0]),spaceA.eightCoordinate[0][1],spaceA.eightCoordinate[0][2],spaceA.spaceLength + spaceB.spaceLength,spaceA.spaceWidth,spaceA.spaceHigh,problem);
                    // 移除合并了的空间
                    spaceArrayList.remove(i);
                    spaceArrayList.remove(j);
                    mergedSpaceList.add(mergedSpace);
                    System.out.println("空间合并");
                    break;
                    //y 方向合并
                }else if((Arrays.equals(spaceA.eightCoordinate[0],spaceB.eightCoordinate[2] )&&
                        Arrays.equals(spaceA.eightCoordinate[1],spaceB.eightCoordinate[3] )&&
                        Arrays.equals(spaceA.eightCoordinate[4],spaceB.eightCoordinate[6] )&&
                        Arrays.equals(spaceA.eightCoordinate[5],spaceB.eightCoordinate[7])) ||
                        (Arrays.equals(spaceB.eightCoordinate[0],spaceA.eightCoordinate[2] )&&
                        Arrays.equals(spaceB.eightCoordinate[1],spaceA.eightCoordinate[3] )&&
                        Arrays.equals(spaceB.eightCoordinate[4],spaceA.eightCoordinate[6] )&&
                        Arrays.equals(spaceB.eightCoordinate[5],spaceA.eightCoordinate[7]))){
                    Space mergedSpace = new Space(spaceA.eightCoordinate[0][0],Math.min(spaceA.eightCoordinate[0][1],spaceB.eightCoordinate[0][1]),spaceA.eightCoordinate[0][2],spaceA.spaceLength,spaceA.spaceWidth + spaceB.spaceWidth,spaceA.spaceHigh,problem);
                    // 移除合并了的空间
                    spaceArrayList.remove(i);
                    spaceArrayList.remove(j);
                    mergedSpaceList.add(mergedSpace);
                    System.out.println("空间合并");
                    break;
                }else if((Arrays.equals(spaceA.eightCoordinate[0],spaceB.eightCoordinate[4] )&&
                        Arrays.equals(spaceA.eightCoordinate[1],spaceB.eightCoordinate[5] )&&
                        Arrays.equals(spaceA.eightCoordinate[2],spaceB.eightCoordinate[6] )&&
                        Arrays.equals(spaceA.eightCoordinate[3],spaceB.eightCoordinate[7])) ||
                        (Arrays.equals(spaceB.eightCoordinate[0],spaceA.eightCoordinate[4] )&&
                                Arrays.equals(spaceB.eightCoordinate[1],spaceA.eightCoordinate[5] )&&
                                Arrays.equals(spaceB.eightCoordinate[2],spaceA.eightCoordinate[6] )&&
                                Arrays.equals(spaceB.eightCoordinate[3],spaceA.eightCoordinate[7]))){
                    Space mergedSpace = new Space(spaceA.eightCoordinate[0][0],spaceA.eightCoordinate[0][1],Math.min(spaceA.eightCoordinate[0][2],spaceB.eightCoordinate[0][2]),spaceA.spaceLength,spaceA.spaceWidth,spaceA.spaceHigh + spaceB.spaceHigh,problem);
                    // 移除合并了的空间
                    spaceArrayList.remove(i);
                    spaceArrayList.remove(j);
                    mergedSpaceList.add(mergedSpace);
        ...
        spaceArrayList.addAll(mergedSpaceList);
    ...
    */
    public static void renewSpaceList(Block block, Space space, ArrayList<Space> spaceArrayList, Problem problem){
        // 把块放在空间的anchor点,根据空间anchor点坐标更新块的八点坐标
        block.initCoordinateAndRangeByAnchorCorner(space);
        // 根据八点坐标得到当前被放置空间的剩余空间
        ArrayList<Space> newSpaceList = getLeftSpaceForCoveredSpace(space,block,problem);
        // 更新 其余交叠空间
    ...
    */
    public static void detectSpatialPhaseInclusions(ArrayList<Space> spaceArrayList){
        ...
        for (int i = 0; i < spaceArrayList.size(); i++) {
            Space spaceA = spaceArrayList.get(i);
            for (Space spaceB : spaceArrayList) {
                if (!spaceA.equals(spaceB) && isInSpace(spaceB, spaceA)) {
