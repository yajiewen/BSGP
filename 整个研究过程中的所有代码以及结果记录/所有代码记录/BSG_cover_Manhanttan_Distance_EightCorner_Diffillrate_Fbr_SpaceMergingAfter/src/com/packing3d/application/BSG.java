...
/**
 * @Author yajiewen
 * @Date 2022-09-24 13-22-02
 * @Description greedy 函数，更新了找不到适合的块的时候，先不把空间删除而是合并后，再找块，还在找不到再删除
*/
...
//            ArrayList<Block> viableBlockList = BlockHelper.searchViableBlockWithNumLimitAndFbr(state.spaceArrayList.get(selectedSpaceIndex), state,1);
            ArrayList<Block> viableBlockList = BlockHelper.myBlockSearchFunction(state,state.spaceArrayList.get(selectedSpaceIndex),1);
            if(!viableBlockList.isEmpty()){
                State.renewState(state,viableBlockList.get(0),selectedSpaceIndex);
            }else{
                //进行空间合并
                ResourceHelper.mergeSpace(state.spaceArrayList,state.problem);
                // 选择空间内
                selectedSpaceIndex = SpaceHelper.selectSpace(state.spaceArrayList);
                // 查找可行快
                viableBlockList = BlockHelper.myBlockSearchFunction(state,state.spaceArrayList.get(selectedSpaceIndex),1);
//                viableBlockList = BlockHelper.searchViableBlockWithNumLimitAndFbr(state.spaceArrayList.get(selectedSpaceIndex), state,1);
                if(!viableBlockList.isEmpty()){
                    State.renewState(state,viableBlockList.get(0),selectedSpaceIndex);
                }else{
...
/**
 * @Author yajiewen
 * @Date 2022-09-24 13-30-14
 * @Description expend 扩展节点算法，增加了找不到块之后进行空间合并
*/
...
//        ArrayList<Block> viableBlockList = BlockHelper.searchViableBlockWithNumLimitAndFbr(state.spaceArrayList.get(selectedSpaceIndex), state, w);
        ArrayList<Block> viableBlockList = BlockHelper.myBlockSearchFunction(state,state.spaceArrayList.get(selectedSpaceIndex),w);
        if(!viableBlockList.isEmpty()){
            ...
            selectedSpaceIndex = SpaceHelper.selectSpace(state.spaceArrayList);
            // 节点扩展
//            viableBlockList = BlockHelper.searchViableBlockWithNumLimitAndFbr(state.spaceArrayList.get(selectedSpaceIndex), state, w);
          viableBlockList = BlockHelper.myBlockSearchFunction(state,state.spaceArrayList.get(selectedSpaceIndex),w);
            for(Block block :viableBlockList){
                State newState = State.getNewState(state,block,selectedSpaceIndex);
                succ.add(newState);
            }
