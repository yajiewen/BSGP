...
public class StateHelper {
    ...
    */
    public static void boxsDetecte(State state, Problem problem){
        ...
        /**
         * @Author yajiewen
         * @Date 2022-09-30 21-40-16
         * @Description 修复块旋转后有的块检测不到的问题
        */
        for(Block block : blockList){
            boolean isFound = false;
            for(Block block1 : state.blockList){
                if(((block.blockLenght == block1.blockLenght && block.blockWidth == block1.blockWidth) || (block.blockLenght == block1.blockWidth && block.blockWidth == block1.blockLenght) )&& block.blockHigh == block1.blockHigh && Arrays.equals(block.neededBoxNumOfEachType, block1.neededBoxNumOfEachType)){
                    isFound = true;
                    break;
                }
            }
