...
//        oneThread();
    ...
    public static void oneThread() throws IOException {
        ...
        FileHelper fileUtils = new FileHelper("C:\\userfile\\论文\\装箱问题\\三维装箱\\经典标准数据集\\七个数据集以及说明文档\\br15.txt");
        ArrayList<Problem> problemList = fileUtils.getProblems(); // 获取文件中的所有问题

        double avaFillRate = 0;
        int i = 1;
        for (Problem problem : problemList) {
            //ArrayList<Block> simpleBlockList = getSimpleBlockList(problem); //简单块(不带方向)
            //ArrayList<Block> complexBlockList = getComplexBlockList(simpleBlockList,MaxLevel,MaxBlocks,problem,MinFillRate,MinAreaRate); //复杂快
...
//                        ArrayList<Block> blockList = BlockHelper.generateComplexBlocksFullySupported(simpleBlockList,MaxLevel,MaxBlocks,problem,MinFillRate,MinAreaRate);
    ...
    public static Scheme getSchemeByHeuristic(ArrayList<Block> blockList, Problem problem, Ps ps){
        ...
        Space space = new Space(0,0,0,problem.containnerLength, problem.containnerWidth, problem.containnerHigh);
        spaceArrayList.add(space);
        // 创建一个方案
        Scheme scheme = new Scheme();
        int index = 0; // 表示第几次放置
        while(!spaceArrayList.isEmpty()){
            space = spaceArrayList.remove(spaceArrayList.size() - 1);
            ...
            if(!viableBlockList.isEmpty()){
                ...
                // 更新资源(箱子,和空间)
                ResourceHelper.renewResource(spaceArrayList,space,selectedBlock,availNum);
            }
        }
        ps.psLength = index;
        return scheme;
