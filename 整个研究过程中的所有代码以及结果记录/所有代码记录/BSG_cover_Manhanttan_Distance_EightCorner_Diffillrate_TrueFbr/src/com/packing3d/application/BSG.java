...
//        oneThread();
        System.out.println("Fbr\n" +
                ...
                "30秒\n" +
                "fasterFbr");
        mutilThread(args[0], args[1], 30, 3);
    }
    ...
    */
    public static void oneThread() throws IOException{
        // 读取文件
        FileHelper fileHelper = new FileHelper("B:\\Standard_Datasets\\br13.txt");
        ...
        for(Problem problem : problems){
            ...
            double rate = state.scheme.totalVolum / problem.containnerVolume;
            System.out.println("利用率=====>" + rate );
            StateHelper.stateDetecte(state,problem);
            System.out.println("put数量:" + state.scheme.putList.size());
            System.gc();
            try {
                Thread.sleep(1000);
...
//        double MinFillRate = 0.98;
//        State state = algorithmBSG(problem,150 * 1000,MinFillRate,false);
...
//        StateHelper.stateDetecte(state,problem);
    ...
    */
    public static State algorithmBSG(Problem problem,int timeLimit,double MinFillRate, boolean isRuoYiGou){
        ...
        ArrayList<Block> blockList = BlockGenerator.guillotineBlocks(problem,MaxBlocks,MinFillRate);
//        if(isRuoYiGou){
//            blockList = BlockGenerator.generateGuillotineBlocksNotFullySupported(problem,MaxBlocks,MinFillRate);
//        }else{
//            blockList = BlockGenerator.generateGeneralCuttingBlocksNotFullySupported(problem,MaxBlocks,MinFillRate);
//        }
...
//        SchemeHelper.schemeDetecte(bestState[0].scheme, bestState[0].problem);
    ...
    */
    public static double greedy(State state,final State[] bestState,State oriState){
        while(!state.spaceArrayList.isEmpty()){
            ...
            ArrayList<Block> viableBlockList = BlockSelector.searchViableBlockWithNumLimitByFbrFaster(state.spaceArrayList.get(selectedSpaceIndex), state,1);
    ...
    */
    public static ArrayList<State> expand(State state, int w){
        ...
        int selectedSpaceIndex = SpaceHelper.selectSpace(state.spaceArrayList);
        ArrayList<Block> viableBlockList = BlockSelector.searchViableBlockWithNumLimitByFbrFaster(state.spaceArrayList.get(selectedSpaceIndex), state, w);
